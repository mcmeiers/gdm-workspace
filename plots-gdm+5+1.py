...
# %%
df = pd.read_csv("min_gdn_5+1_ttteee_scipy_Nncdm.csv")

param_sets = {
    row["gdm_alpha"]: {k: row[k] for k in model.parameterization.sampled_params()}
...
H = {}
bgs = {}
for alpha, ps in param_sets.items():
    likelihoods[alpha] = model.logposterior(ps, as_dict=True)["loglikes"]
    likesums[alpha] = sum(likelihoods[alpha].values())
    Cls[alpha] = model.provider.get_Cl(ell_factor=True)
    H[alpha] = model.provider.get_Hubble(redshifts)
...
for alpha in param_sets.keys():
    ax_h.plot(redshifts, H[alpha], label=r"$\alpha=$" + str(alpha))
...
for alpha in param_sets.keys():
    if alpha != 0:
        ax_h.plot(
            redshifts, (H[alpha] - H[0.0]) / H[0.0], label=r"$\alpha=$" + str(alpha)
        )
...
for alpha in param_sets.keys():
    if alpha != 0:
        zmask = (bgs[alpha]["z"] >= 0) & (bgs[alpha]["z"] <= 10**7)
        ax_h.plot(
            np.log(bgs[alpha]["z"][zmask]),
            (bgs[alpha]["(.)rho_gdm"] / bgs[alpha]["H [1/Mpc]"] ** 2)[zmask],
