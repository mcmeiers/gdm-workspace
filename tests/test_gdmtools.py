...
class Test_wModel:
    ...
    def test_wModel_init_range_of_param_default(self):
        w_mdl = gdmtools.wModel([-1, 0])
        expected_range_of_param = dict(
            zip(["w_0", "w_1"], ({"min": -1, "max": 1},) * 2)
        )
    ...
    def test_wModel_knots_w_vals_expected_output_fixed(self):
        ...
        assert all(w_mdl.knots_w_vals(1 / 3.0) == np.array([1, 1 / 3.0]))

    def test_wModel_classy_fmt_knots_w_vals_signature(self):
        w_mdl = gdmtools.wModel([-1, 0])
        assert w_mdl.classy_fmt_knots_w_vals.__code__.co_varnames[
            : w_mdl.knots_w_vals.__code__.co_argcount
        ] == ("w_0", "w_1")
    ...
    def test_wModel_classy_fmt_knots_w_vals_expected_output_with_fixed(self):
        w_mdl = gdmtools.wModel([-1, 0], [(0, 1)])
        assert w_mdl.classy_fmt_knots_w_vals(1 / 3.0) == f"{1},{1 /3.}"

    def test_wModel_w_mins(self):
        range_filter = ((-1 / 2.0, {"min": -1 / 2, "max": 1 / 2}),)
        w_mdl = gdmtools.wModel([-1, 0], range_filter=range_filter)
        expected_w_mins = [-1, -1 / 2]
        assert w_mdl.w_mins == expected_w_mins

    def test_wModel_w_maxes(self):
        range_filter = ((-1 / 2.0, {"min": -1 / 2, "max": 1 / 2}),)
        w_mdl = gdmtools.wModel([-1, 0], range_filter=range_filter)
        expected_w_maxes = [1, 1 / 2]
        assert w_mdl.w_maxes == expected_w_maxes
...
class Test_gdmModel:
    def test_gdm_model_fixed_settings(self):
        ...
        gdm_mdl = gdmtools.gdmModel(
            w_mdl, alpha_range, c_eff2=c_eff2, c_vis2=c_vis2, z_alpha=z_alpha
        )
        expected_settings = {
            "gdm_log10a_vals": "-1,0",
            "gdm_c_eff2": 1 / 7,
