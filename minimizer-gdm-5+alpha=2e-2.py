...
def main(args):
    ...
    updated_info, sampler = run(cobaya_info, debug=True)
    ...
    if attempt == 0:
        df.to_csv("min_gdm_alpha=2e-2-trials-meth=bob.csv", mode="w", index=False)
    else:
        df.to_csv(
            "min_gdm_alpha=2e-2-trials-meth=bob.csv",
            mode="a",
            index=False,
            header=False,
        )
